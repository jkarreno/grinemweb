...
function clientes($vendedor='todos')
{
	...
	{	
		...
		$cadena.='<tr id="row_'.$A.'" style="background: '.$bgcolor.'">
					...
					<td align="left" class="texto" style="border:1px solid #FFFFFF" onmouseover="row_'.$A.'.style.background=\'#00CED1\'" onmouseout="row_'.$A.'.style.background=\''.$bgcolor.'\'">';if($_SESSION["perfil"]!='AgenteV' AND $_SESSION["perfil"]!='produccion'){$cadena.='<a href="#" onclick="xajax_editar_cliente(\''.$RResClientes["Id"].'\')"><img src="images/edit.png" border="0"></a>';}$cadena.='</td>
				</tr>';
		$A++; $I++;
...
function cobranza($accion=NULL, $form=NULL, $vendedor='todos')
{
	...
	if($_SESSION["perfil"]!='AgenteV'){$ResClientesA=mysql_query("SELECT Id, Nombre, Adeudo, Vencido, DeudaVencida FROM clientes WHERE Juridico='0' AND Tipo='A' ORDER BY DeudaVencida DESC");}
	...
	$cadena.='			<form name="fsumaaexistencias" id="fsumaaexistencias">
							<table border="0" style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="center">
								...
								<tr>
									<td colspan="5" bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">EXISTENCIA PANTALONES</td>
								</tr>
								<tr>
