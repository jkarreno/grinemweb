...
function pagos($cliente=NULL, $accion=NULL, $form=NULL, $anno='todos')
{
	...
	{	
		//Agregamos venta
		case 'adventa':
			$comentariosSQL = htmlentities(addslashes($form["comentarios"]));
			mysql_query("INSERT INTO ventas (Cliente, Fecha, Nota, Apagar, Importe, Provedor, Comision, SComision, Comentarios, checka, Metros)
									 VALUES ('".$cliente."',
											 ...
											 '".$form["comision"]."',
											 '".$form["scomision"]."',
											 '".utf8_decode(htmlentities(addslashes($form["comentarios"])))."',
											 '".$form["checka"]."',
		...
		case 'editventa':
			$comentariosSQL = htmlentities(addslashes($form["comentarios"]));
			$form["historico"] = ($form["historico"]=='') ? 0 : $form["historico"];
			mysql_query("UPDATE ventas SET Fecha='".$form["anno"]."-".$form["mes"]."-".$form["dia"]."',
										   ...
										   Status='".$form["status"]."',
										   Comentarios='".utf8_decode(htmlentities(addslashes($form["comentarios"])))."',
										   checka='".$form["checka"]."',
		...
		//agregar Pago
		case 'adpago':
			mysql_query("INSERT INTO pagos (Cliente, Banco, Cheque, Fecha, Comision, Pago, Comentarios, checka, Metros)
									VALUES ('".$cliente."',
											'".$form["banco"]."',
											'".$form["numcheque"]."',
											'".$form["anno"]."-".$form["mes"]."-".$form["dia"]."',
											'".$form["comision"]."',
											'".$form["pago"]."',
											'".nl2br(addslashes(utf8_decode($form["comentarios"])), false)."',
											'".$form["checka"]."',
											'".$form["metros"]."')") or die(mysql_error());
			...
			if($form["banco"]==94)
			{
				mysql_query("INSERT INTO compras (Fecha, Metros) VALUES ('".$form["anno"]."-".$form["mes"]."-".$form["dia"]."', '".$form["metros"]."')");
			}
		//aplicarpago
		case 'aplicapago':
		...
		case 'editpago':
			mysql_query("UPDATE pagos SET Banco='".$form["banco"]."', 
										   ...
										   Pago='".$form["pago"]."',
										   Status='".$form["status"]."',
										   Comentarios='".nl2br(addslashes(utf8_decode($form["comentarios"])), false)."',
										   checka='".$form["checka"]."',
										   Historico='".$form["historico"]."',
										   AnnoHistorico='".$form["annohistorico"]."',
										   Metros='".$form["metros"]."'
	...
	{
		$cadena.='<td valign="top" class="texto">
					Tela: <select name="telas" id="telas" onchange="xajax_existencias(this.value)"><option>SELECCIONE</option>';
	...
	{	
		...
		$comision= $RResVentas["Importe"] * ($RResVentas["Comision"]/100);
        $comisionIVA = $comision * 0.16;
        $comisionRET = $comision * 0.1067;
        $TotalComision = number_format(  $comision + $comisionIVA - $comisionRET ,2 );
		...
		$cadena.='			<tr>
								...
								<td bgcolor="'.$bgcolor.'" align="right" class="texto" style="border:1px solid #FFFFFF">$ '.$TotalComision.'</td>
							</tr>';
		}
		$status='';
	}
...
//////// PANTALLA HOME /////////
	elseif($cliente==NULL)
	{
		...
		if($_SESSION["perfil"]!='AgenteV'){$ResClientesA=mysql_query("SELECT * FROM clientes WHERE Juridico='0' AND Adeudo <> 0 ORDER BY Adeudo DESC");}
		else{$ResClientesA=mysql_query("SELECT * FROM clientes WHERE Juridico='0' AND Vendedor='".$_SESSION["idusuario"]."' ORDER BY Adeudo DESC");}
		$A=1; $B=1;
...
$cadena.='				</select>
				...
				<tr>
					<td colspan="10" bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF">Comentarios: <br /><textarea name="comentarios" id="comentarios" cols="50" rows="5">'.str_replace('<br>', "", $ResVenta["Comentarios"]).'</textarea></td>
				</tr>
				</tr>
				<tr>
...
$cadena.='				</select>
				...
				<tr>
					<td colspan="9" bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">
						<textarea name="comentarios" id="comentarios" rows="5" cols="50">'.str_replace('<br>', "", $ResPago["Comentarios"]).'</textarea>
				</tr>
				<tr>
					<td colspan="8" bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF">
						<input type="hidden" name="idpago" id="idpago" value="'.$ResPago["Id"].'">';
...
function cheque_pago($cheque)
{
	...
	$cadena.='		</select>
				...
				<tr>
					<td colspan="5" bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF">
						<input type="hidden" name="idcheque" id="idcheque" value="'.$ResCheque["Id"].'">';
	if($ResCheque["checka"]==1 AND $_SESSION["perfil"]=='administra'){$cadena.='<input type="submit" name="boteditchequepago" id="boteditchequepago" value="Editar>" onclick="lightbox.style.visibility=\'hidden\'; xajax_pagos(\''.$ResCheque["Cliente"].'\', \'adpago\', xajax.getFormValues(\'fchequepago\')); document.getElementById(\'lightbox\').innerHTML = \'\'">';}
	$cadena.='		</td>
				</tr>
			</table>
			</form>';
...
function pagos_provedor($provedor=NULL, $accion=NULL, $form=NULL, $anno='todos')
{
	...
	$cadena='<p align="center" class="texto">'.$hola.' Cliente: <select name="cliente" id="cliente" onchange="xajax_pagos(this.value)"><option>Seleccione</option>';
	...
	$cadena.='<table border="0" align="center">
				<tr>
					<td valign="top" align="center">
					<form name="fsumacomprasp" id="fsumacomprasp">
						<table border="0" style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="center">
							<tr>
								<td bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF">&nbsp;</td>
								<td bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF"><a href="#" onclick="xajax_pagos_provedor(\''.$provedor.'\', \'suma\', xajax.getFormValues(\'fsumacomprasp\'))">Calcular</a></td>
								<td bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF">';if($suma>0){$cadena.='$ '.number_format($suma,2);}$cadena.='</td>
							...
							<tr>
                                <td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF"><input type="checkbox" name="allsumacomprasp" id="allsumacomprasp" value="1"';if($form["allsumacomprasp"]==1){$cadena.=' checked';}$cadena.=' onchange="seleccionar_todo_sumacomprasp()"></td>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">FECHA</td>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">IMPORTE</td>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">SALDO</td>
...
function editar_compra($compra)
{
	...
	$cadena='<form name="feditcompra" id="feditcompra">
			<table border="0" style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="center">
				<tr>
					<td colspan="7" align="right" class="texto">[ <a href="#" onclick="lightbox.style.visibility=\'hidden\';">X</a> ]</td>
				</tr>
...
$cadena.='				</select>
				...
				<tr>
					<td colspan="7" bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF">Comentarios: <br /><textarea name="comentarios" id="comentarios" cols="50" rows="5">'.str_replace('<br>', "", $ResCompra["Comentarios"]).'</textarea></td>
				</tr>
				<tr>
					<td colspan="7" bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF">
						<input type="hidden" name="idcompra" id="idcompra" value="'.$ResCompra["Id"].'">';
if($ResCompra["checka"]==1 AND $_SESSION["perfil"]=='administra'){$cadena.='<input type="button" class="boton" name="boteditcompra" id="boteditcompra" value="Editar>" onclick="lightbox.style.visibility=\'hidden\'; xajax_pagos_provedor(\''.$ResCompra["Provedor"].'\', \'editcompra\', xajax.getFormValues(\'feditcompra\')); document.getElementById(\'lightbox\').innerHTML = \'\'">';}
elseif($ResCompra["checka"]==0 AND ($_SESSION["perfil"]=='administra' OR $_SESSION["perfil"]=="usuario")){$cadena.='<input type="button" class="boton" name="boteditcompra" id="boteditcompra" value="Editar>" onclick="lightbox.style.visibility=\'hidden\'; xajax_pagos_provedor(\''.$ResCompra["Provedor"].'\', \'editcompra\', xajax.getFormValues(\'feditcompra\')); document.getElementById(\'lightbox\').innerHTML = \'\'">';}
$cadena.='			</td>
			</table>
			</form>';
...
$cadena.='				</select>
				...
				<tr>
					<td colspan="7" bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">
						<textarea name="comentarios" id="comentarios" rows="3" cols="50">'.str_replace('<br>', "", $ResPago["Comentarios"]).'</textarea>
					</td>
				</tr>
				<tr>
					<td colspan="7" bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF">
