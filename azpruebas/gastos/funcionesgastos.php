...
function gastos_jack($accion=NULL, $form=NULL, $anno)
{
	...
	$cadena='<form name="fgastos" id="fgastos"><table style="border:1px solid #FFFFFF" cellpadding="1" cellspacing="0" align="center">
				<tr>
					<td colspan="15" bgcolor="#ffffff" align="right" class="texto" style="border:1px solid #FFFFFF">| <a href="#" onclick="lightbox.style.visibility=\'visible\'; xajax_agregar_gasto_jack(\''.$anno.'\')">Agregar Gasto</a> |</td>
					<td bgcolor="#FFFFFF" align="right" class="texto" style="border:1px solid #FFFFFF">&nbsp;</td>
				</tr>
				<tr>
					<td colspan="15" bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">JACK GASTOS MENSUALES <select name="anno" id="anno" onchange="xajax_gastos_jack(\'\',\'\',this.value)">';
	...
	$ResGastos=mysql_query("SELECT g.Id,g.Concepto from gastos as g, gastos_montos as gm where g.Id=gm.IdGasto and g.Tipo='jack' and gm.Fecha LIKE '".$anno."%' GROUP BY g.id, g.Concepto ORDER BY g.Concepto");
...
function agregar_gasto_jack($anno)
{
			...
			<table style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="center">
				...
				<tr>
					...
					<td class="texto" align="left" bgcolor="#CCCCCC" style="border:1px solid #FFFFFF"><select name="mes" id="mes">
						...
						</select> <select name="anno" id="anno"><option value="0000">A�o</option>';
	for($i=2014; $i<=(date("Y")+2); $i++)
	{
		$cadena.='<option value="'.$i.'"';if($i==$anno){$cadena.=' selected';}$cadena.='>'.$i.'</option>';
	}
	$cadena.='			</select></td>
				...
				<tr>
					...
					<td class="texto" align="left" bgcolor="#CCCCCC" style="border:1px solid #FFFFFF"><select name="diav" id="diav"><option value="00">D�a</option>';
	for($i=1; $i<=31; $i++)
	{
		if($i<=9){$i='0'.$i;}
...
if($ResMonto["checka"]==1 AND $_SESSION["perfil"]=='administra'){$cadena.='<input type="button" name="boteditgastojack" id="boteditgastojack" value="Editar>>" class="boton" onclick="lightbox.style.visibility=\'hidden\';  xajax_gastos_jack(\'editgastojack\', xajax.getFormValues(\'feditgastojack\'), \''.$anno.'\'); document.getElementById(\'lightbox\').innerHTML = \'\'">';}
elseif($ResMonto["checka"]==0 AND ($_SESSION["perfil"]=='administra' OR $_SESSION["perfil"]=="usuario")){$cadena.='<input type="button" name="boteditgastojack" id="boteditgastojack" value="Editar>>" class="boton" onclick="lightbox.style.visibility=\'hidden\';  xajax_gastos_jack(\'editgastojack\', xajax.getFormValues(\'feditgastojack\'), \''.$anno.'\'); document.getElementById(\'lightbox\').innerHTML = \'\'">';}
...
function editar_gasto_jack_nuevo_monto($gasto, $anno)
{
	...
	$cadena.='			</select></td>
				...
				<tr>
					<td class="texto" align="center" colspan="2" bgcolor="#CCCCCC" style="border:1px solid #FFFFFF">
						<input type="hidden" name="idgasto" id="idgasto" value="'.$ResGasto["Id"].'">';
if($ResVenta["checka"]==1 AND $_SESSION["perfil"]=='administra'){$cadena.='<input type="button" name="boteditgastojack" id="boteditgastojack" value="Editar>>" class="boton" onclick="lightbox.style.visibility=\'hidden\';  xajax_gastos_jack(\'editgastojacknuevomonto\', xajax.getFormValues(\'feditgastojack\'), \''.$anno.'\'); document.getElementById(\'lightbox\').innerHTML = \'\'">';}
elseif($ResVenta["checka"]==0 AND ($_SESSION["perfil"]=='administra' OR $_SESSION["perfil"]=="usuario")){$cadena.='<input type="button" name="boteditgastojack" id="boteditgastojack" value="Editar>>" class="boton" onclick="lightbox.style.visibility=\'hidden\';  xajax_gastos_jack(\'editgastojacknuevomonto\', xajax.getFormValues(\'feditgastojack\'), \''.$anno.'\'); document.getElementById(\'lightbox\').innerHTML = \'\'">';}
elseif($ResVenta["checka"]==1 AND $_SESSION["perfil"]=="usuario"){$cadena.='';}
		$cadena.='</td>
				</tr>
			</table>
			</form>';
