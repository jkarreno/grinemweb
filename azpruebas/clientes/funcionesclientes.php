<?php
function clientes($vendedor='todos', $nombrecliente)
{
	include ("conexion.php");
	
	$cadena='<table style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="center">
				<tr>
					<td colspan="14" bgcolor="#ffffff" align="left" class="texto" style="border:1px solid #FFFFFF">Vendedor: 
                    
                    
                    <select name="vendedor" id="vendedor" onchange="xajax_clientes(document.getElementById(\'vendedor\').value, document.getElementById(\'nombrecliente\').value)">
	...
	$cadena.='		</select>
                    ...
                    &nbsp Nombre del cliente: <input type="text" id="nombrecliente" name="nombrecliente" class="texto" value="'.$nombrecliente.'">
                    
                    <a href="#" onclick="xajax_clientes(document.getElementById(\'vendedor\').value, document.getElementById(\'nombrecliente\').value)"/>Buscar</a>
                </td>
                </tr>
	...
	if($_SESSION["perfil"]!='AgenteV')
    {
        if($vendedor=='todos')
        {$ResClientes=mysql_query("SELECT * FROM clientes WHERE Juridico='0' AND Nombre LIKE '%".$nombrecliente."%'  ORDER BY Nombre ASC");}
        else
        {$ResClientes=mysql_query("SELECT * FROM clientes WHERE Juridico='0' AND Vendedor='".$vendedor."' AND Nombre LIKE '%".$nombrecliente."%' ORDER BY Nombre ASC");}
    }
	else
    {$ResClientes=mysql_query("SELECT * FROM clientes WHERE Juridico='0' AND Vendedor='".$_SESSION["idusuario"]."' AND Nombre LIKE '%".$nombrecliente."%'  ORDER BY Nombre ASC");}
	$bgcolor="#CCCCCC"; $A=1; $I=1;
	while($RResClientes=mysql_fetch_array($ResClientes))
	{	
		$ResVendedor=mysql_fetch_array(mysql_query("SELECT Nombre FROM usuarios WHERE Id='".$RResClientes["Vendedor"]."' AND Nombre LIKE '%".$nombrecliente."%'  LIMIT 1"));
        ...
        if($RResClientes["CheckEdicion"] == 0)
        {
...
$cadena.='				</select>
        ...
        if($RResClientes["CheckEdicion"] == 0)
        {
            ...
            if($_SESSION["sadmon"] == 1)
            {
				...
				<tr>
					<td colspan="2" class="texto" align="center" bgcolor="#CCCCCC" style="border:1px solid #FFFFFF">
						<input type="checkbox" name="editor" id="editor" value="1"';if($ResCliente["CheckEdicion"]==1){$cadena.=' checked';}$cadena.='> Check
					</td>
				</tr>';
...
function cobranza($accion=NULL, $form=NULL, $vendedor='todos')
{
	...
	while($RResClientesA=mysql_fetch_array($ResClientesA))
	{
		...
		// {
			$RResVencidas=mysql_fetch_array(mysql_query("SELECT SUM(Importe) AS ImporteTotal FROM ventas WHERE Cliente='".$RResClientesA["Id"]."' AND Status='VENCIDA'"));
			
			//$ResPago=mysql_query("SELECT * FROM  `pagos` WHERE Cliente =  '499'AND Historico =  '0'");
