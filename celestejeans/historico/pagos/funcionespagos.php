...
function pagos($cliente=NULL, $accion=NULL, $form=NULL, $anno='todos', $annohist='todos')
{
	...
	$ResAvios=mysql_query("SELECT Id, Nombre FROM avios WHERE Status='1' ORDER BY Nombre ASC");
...
///adeudo a provedores
	...
	$ResTelas=mysql_query("SELECT Id, Nombre FROM telas WHERE Status='1' ORDER BY Nombre ASC");
	while($RResTelas=mysql_fetch_array($ResTelas))
	{
		$cadena.='<option value="'.$RResTelas["Id"].'">'.$RResTelas["Nombre"].'</option>';
	}
	...
	$cadena.='</select> <img src="images/impresora.jpg" border="0">
		...
		$A=1;
		$ResAvios=mysql_query("SELECT Id, Nombre, Color, Existencia, Precio, Importe FROM avios WHERE Existencia>0 AND Status='1' ORDER BY Nombre ASC");
		while($RResAvios=mysql_fetch_array($ResAvios))
		{
			$cadena.='<tr>
