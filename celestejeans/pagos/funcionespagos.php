<?php
function pagos($cliente=NULL, $accion=NULL, $form=NULL, $anno='todos')
{
	...
	{	
		//Agregamos venta
		case 'adventa':
			mysql_query("INSERT INTO ventas (Cliente, Fecha, Nota, Apagar, Importe, Provedor, Comision, SComision, Comentarios, checka, Metros)
									 VALUES ('".$cliente."',
											 ...
											 '".$form["comentarios"]."',
											 '".$form["checka"]."',
											 '".$form["metros"]."')") or die(mysql_error());
			break;
		//pagar venta
		case 'pagaventa':
		...
		case 'sumaatelas':
			...
			break;
		//suma montos de avios	
		case 'sumavios':
			$sumavios=0;
			$ResSumaAvios=mysql_query("SELECT Id, Existencia, Precio FROM avios ORDER By Id ASC");
			while($RResSumaAvios=mysql_fetch_array($ResSumaAvios))
			{
				if($form["aavio_".$RResSumaAvios["Id"]]==1)
				{
					$sumavios=$sumavios+($RResSumaAvios["Existencia"]*$RResSumaAvios["Precio"]);
				}
			}
			break;
	}
	if($cliente!=NULL)
	{
		$cadena='<table border="0" cellpadding="3" align="center">
					<tr>
						<td valign="top">
						<p align="center" class="texto">Cliente: <select name="cliente" id="cliente" onchange="xajax_pagos(this.value)"><option>SELECCIONE</option>';
		if($_SESSION["perfil"]!='AgenteV'){$ResCliente=mysql_query("SELECT * FROM clientes WHERE Juridico='0' ORDER BY Nombre ASC");}
		else{$ResCliente=mysql_query("SELECT * FROM clientes WHERE Juridico='0' AND Vendedor='".$_SESSION["idusuario"]."' ORDER BY Nombre ASC");}
		...
		$cadena.='</select> <a href="reportes/reportecliente.php?cliente='.$cliente.'" target="_blank"><img src="images/impresora.jpg" border="0"></a></p>
		...
		{
			$cadena.='<p align="center" class="texto">Provedor: <select name="provedor" id="provedor" onchange="xajax_pagos_provedor(this.value)"><option>SELECCIONE</option>';
			$ResProvedor=mysql_query("SELECT Id, Nombre FROM provedores WHERE Status='0' ORDER BY Nombre ASC");
			while($RResProvedor=mysql_fetch_array($ResProvedor))
			{
				$cadena.='<option value="'.$RResProvedor["Id"].'">'.$RResProvedor["Nombre"].'</option>';
			}
			$cadena.='</select> <img src="images/impresora.jpg" border="0"></p>';
		}
	...
	$cadena.='
		...
		{		  
			$cadena.='<td valign="top">
						<p align="center" class="texto">Modelo: <select name="telas" id="telas" onchange="xajax_existencias(this.value)"><option>SELECCIONE</option>';
			$ResTelas=mysql_query("SELECT Id, Nombre FROM telas WHERE Status='0' ORDER BY Nombre ASC");
			...
			$cadena.='</select> <img src="images/impresora.jpg" border="0">
						...
						<td valign="top">
						<p align="center" class="texto">Avios: <select name="avios" id="avios" onchange="xajax_existencias_avios(this.value)"><option>SELECCIONE</option>';
			...
			$cadena.='</select>	<img src="images/impresora.jpg" border="0"></p>
					...
					<tr>
						<td valign="top">';
		$ResDatos=mysql_fetch_array(mysql_query("SELECT EncargadoPagos, TelPagos, CorreoPagos FROM clientes WHERE Id='".$cliente."' LIMIT 1"));
		$cadena.='			<p align="left" class="texto">Contacto: '.$ResDatos["EncargadoPagos"].'<br />
							Telefono: '.$ResDatos["TelPagos"].'<br />	
							Correo: '.$ResDatos["CorreoPagos"].'</p>
						</td>
						<td valign="top">';
		...
		$cadena.='			<p class="texto">Limite de Credito: $ '.number_format($ResCredito["Credito"],2).'
				<br />&nbsp;<br />A&ntilde;o: <select name="anno" id="anno" onChange="xajax_pagos(\''.$cliente.'\', \''.$accion.'\', \''.$form.'\', this.value)">
					<option value="todos">Todos</option>';
	for($i=2000; $i<=(date("Y")+2); $i++)
	{
		$cadena.='<option value="'.$i.'"';if($anno==$i){$cadena.=' selected';}$cadena.='>'.$i.'</option>';
	}	
	$Vendedor=mysql_fetch_array(mysql_query("SELECT Vendedor FROM clientes WHERE Id='".$cliente."' LIMIT 1"));
	$ResVendedor=mysql_fetch_array(mysql_query("SELECT Nombre FROM usuarios WHERE Id='".$Vendedor["Vendedor"]."' LIMIT 1"));
	$cadena.='	</select>
				</p>
						</td>
						<td valign="top">
							<p class="texto">Vendedor: '.$ResVendedor["Nombre"].'</p>
						</td>
						<td valign="top">
						</td>
					</tr>
					...
					<form name="fsumaventas" id="fsumaventas">
						<table border="0" style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="center">
							<tr>
								...
								<td bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF"><a href="#" onclick="xajax_pagos(\''.$cliente.'\', \'suma\', xajax.getFormValues(\'fsumaventas\'), \''.$anno.'\')">Calcular:</a>	</td>
							...
							<tr>
								<td colspan="7" bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">VENTAS</td>
								<td bgcolor="#5263ab" align="center" class="texto" style="border:1px solid #FFFFFF">&nbsp;</td>
							</tr>
							<tr>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF"><input type="checkbox" name="allsumaventas" id="allsumaventas" value="1"';if($form["allsumaventas"]==1){$cadena.=' checked';}$cadena.=' onchange="seleccionar_todo_sumaventas()"></td>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">FECHA</td>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">NOTA</td>
	...
	$ResVentas=mysql_query("SELECT * FROM ventas WHERE Cliente='".$cliente."' AND Historico='0' ORDER BY Fecha ASC");
	$saldo=0;
	while($RResVentas=mysql_fetch_array($ResVentas))
	{
		...
		if(date("Y-m-d")>$nuevafecha)
		{
			...
			if($RResVentas["Status"]=='')
			{
				mysql_query("UPDATE ventas SET Status='VENCIDA' WHERE Id='".$RResVentas["Id"]."'");
			}
			//
		}
	...
	$cadena.='		<td valign="top" align="center">
						...
						<table border="0" style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="center">
							<tr>
								<td colspan="5" align="right" class="texto"><a href="#" onclick="xajax_pagos(\''.$cliente.'\', \'sumapagos\', xajax.getFormValues(\'fsumapagos\'), \''.$anno.'\')"> Calcular:</a></td>
							...
							<tr>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF"><input type="checkbox" name="allsumapagos" id="allsumapagos" value="1"';if($form["allsumapagos"]==1){$cadena.=' checked';}$cadena.=' onchange="seleccionar_todo_sumapagos()"></td>
	...
	$ResPagos=mysql_query("SELECT * FROM pagos WHERE Cliente='".$cliente."' AND Historico='0' ORDER BY Fecha ASC");
	$pago=0;
	while($RResPagos=mysql_fetch_array($ResPagos))
	{
		...
		{
		$cadena.='			<tr>
								<td bgcolor="'.$bgcolor.'" align="center" class="texto" style="border:1px solid #FFFFFF"><input type="checkbox" name="pago_'.$RResPagos["Id"].'" id="pago_'.$RResPagos["Id"].'" value="1"'; if($form["pago_".$RResPagos["Id"]]==1){$cadena.=' checked';}$cadena.='></td>
								...
								<td bgcolor="'.$bgcolor.'" align="right" class="texto" style="border:1px solid #FFFFFF"><a href="#" ';if($_SESSION["perfil"]!='AgenteV' AND $_SESSION["perfil"]!='produccion'){$cadena.='onclick="lightbox.style.visibility=\'visible\'; xajax_editar_pago(\''.$RResPagos["Id"].'\');"';};if($RResPagos["Status"]!='Pagado'){$cadena.=' class="Ntooltip"';}else{$cadena.=' class="Ntooltip0"';}$cadena.='>$ '.number_format($RResPagos["Pago"],2);if($_SESSION["perfil"]!='AgenteV' AND $_SESSION["perfil"]!='produccion'){$cadena.='<span>'.$RResPagos["Comentarios"].'</span>';}$cadena.='</a></td>
	...
	$RResVencidas=mysql_fetch_array(mysql_query("SELECT SUM(Importe) AS ImporteTotal FROM ventas WHERE Cliente='".$cliente."' AND Status='VENCIDA'"));
	$RResNoVencidas=mysql_fetch_array(mysql_query("SELECT SUM(Importe) AS ImporteTotal FROM ventas WHERE Cliente='".$cliente."' AND Status=''"));
	$cadena.='			</table>
					</td>
				</tr>
				<tr>
					...
					<p></p>
						<table border="0" style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="left">
							...
							<tr>
								<td bgcolor="#CCCCCC" align="right" class="texto" style="border:1px solid #FFFFFF">DEUDA NO VENCIDA : </td>
								<td bgcolor="#CCCCCC" align="right" class="texto" style="border:1px solid #FFFFFF">$ '.number_format($RResNoVencidas["ImporteTotal"],2).'</td>
							</tr>
							<tr>
								<td bgcolor="#CCCCCC" align="right" class="texto" style="border:1px solid #FFFFFF">DEUDA VENCIDA: </td>
								<td bgcolor="#CCCCCC" align="right" class="texto" style="border:1px solid #FFFFFF">$ '.number_format((($saldo-$pago)-$RResNoVencidas["ImporteTotal"]),2).'</td>
							</tr>';
	...
	$cadena.='			</table>
			...
			';
//aqui es donde pone a los clientes en vencido
		$adeudo=$saldo-$pago;
		$vencida=$adeudo-$RResVencidas["ImporteTotal"];
		$deuda=($saldo-$pago)-$RResNoVencidas["ImporteTotal"];
		mysql_query("UPDATE clientes SET Adeudo='".$adeudo."', Vencido='".$vencida."', DeudaVencida='".$deuda."' WHERE Id='".$cliente."'") or die(mysql_error());
...
//////// PANTALLA HOME /////////
		...
		$cadena.='<form name="fsumaaclientes" id="fsumaaclientes"><table border="0" style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="center">
					<tr>
						<td colspan="3" align="left" class="texto">
							Cliente: <select name="cliente" id="cliente" onchange="xajax_pagos(this.value)"><option>SELECCIONE</option>';
	if($_SESSION["perfil"]!='AgenteV'){$ResCliente=mysql_query("SELECT * FROM clientes WHERE Juridico='0' ORDER BY Nombre ASC");}
	else{$ResCliente=mysql_query("SELECT * FROM clientes WHERE Juridico='0' AND Vendedor='".$_SESSION["idusuario"]."' ORDER BY Nombre ASC");}
	while($RResCliente=mysql_fetch_array($ResCliente))
	{
	...
	$cadena.='				</select> <a href="reportes/reportecliente.php?cliente='.$cliente.'" target="_blank"><img src="images/impresora.jpg" border="0"></a>
						</td>
					</tr>
					<tr>
		...
		if($_SESSION["perfil"]!='AgenteV'){$ResClientesA=mysql_query("SELECT Id, Nombre, Adeudo FROM clientes WHERE Adeudo>0 AND Juridico='0' ORDER BY Adeudo DESC");}
		else{$ResClientesA=mysql_query("SELECT Id, Nombre, Adeudo FROM clientes WHERE Adeudo>0 AND Juridico='0'  AND Vendedor='".$_SESSION["idusuario"]."' ORDER BY Adeudo DESC");}
		$A=1; $B=1;
...
{	
		$cadena.='<form name="fsumaaprovedores" id="fsumaaprovedores">
				<table border="0" style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="center">
					<tr>
						<td colspan="3" class="texto" align="left">
							Provedor: <select name="provedor" id="provedor" onchange="xajax_pagos_provedor(this.value)"><option>SELECCIONE</option>';
	$ResProvedor=mysql_query("SELECT Id, Nombre FROM provedores WHERE Status='0' ORDER BY Nombre ASC");
	while($RResProvedor=mysql_fetch_array($ResProvedor))
	{
		$cadena.='<option value="'.$RResProvedor["Id"].'">'.$RResProvedor["Nombre"].'</option>';
	...
	$cadena.='</select> <img src="images/impresora.jpg" border="0">
		...
		$ResProvedoresA=mysql_query("SELECT Id, Nombre, Adeudo, TipoCambio FROM provedores WHERE Adeudo>0 AND Status='0' ORDER BY Adeudo DESC");
		$A=1;
...
if($_SESSION["perfil"]!='AgenteV' AND $_SESSION["perfil"]!='produccion')
{
		$cadena.='<form name="fsumaatelas" id="fsumaatelas"><table border="0" style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="center">
					<tr>
						<td colspan="3" class="texto" align="left">
							Modelo: <select name="telas" id="telas" onchange="xajax_existencias(this.value)"><option>SELECCIONE</option>';
	$ResTelas=mysql_query("SELECT Id, Nombre FROM telas WHERE Status='0' ORDER BY Nombre ASC");
	while($RResTelas=mysql_fetch_array($ResTelas))
	{
		$cadena.='<option value="'.$RResTelas["Id"].'">'.$RResTelas["Nombre"].'</option>';
	...
	$cadena.='</select><img src="images/impresora.jpg" border="0">
					...
					<tr>
						<td colspan="6" bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">EXISTENCIA MODELOS</td>
					</tr>
					<tr>
...
{
		$cadena.='	<td valign="top">
						<form name="fsumavios" id="fsumavios"><table border="0" style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="center">
					<tr>
						<td colspan="3" align="left" class="texto">
							Avios: <select name="avios" id="avios" onchange="xajax_existencias_avios(this.value)"><option>SELECCIONE</option>';
	$ResAvios=mysql_query("SELECT Id, Nombre FROM avios WHERE Status='0' ORDER BY Nombre ASC");
	while($RResAvios=mysql_fetch_array($ResAvios))
	{
		$cadena.='<option value="'.$RResAvios["Id"].'">'.$RResAvios["Nombre"].'</option>';
	...
	$cadena.='</select><img src="images/impresora.jpg" border="0">
					...
					<tr>
						<td colspan="5" align="right" class="texto" style="border: 1px solid #FFFFFF"><a href="#" onclick="xajax_pagos(\'\', \'sumavios\', xajax.getFormValues(\'fsumavios\'))">Calcular :</a></td>
						<td align="right" class="texto" style="border: 1px solid #FFFFFF">';if($sumavios>0){$cadena.='$ '.number_format($sumavios,2);}$cadena.='</td>
					</tr>
					<tr>
						<td colspan="6" bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">EXISTENCIA AVIOS</td>
					</tr>
					<tr>
						<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF"><input type="checkbox" name="allavios" id="allavios" value="1" ';if($form["allavios"]==1){$cadena.=' checked';}$cadena.=' onchange="seleccionar_todo_sumaavios()"></td>
						<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">#</td>
						<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">NOMBRE</td>
						<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">COLOR</td>
						<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">EXISTENCIA</td>
		...
		$A=1;
		$ResAvios=mysql_query("SELECT Id, Nombre, Color, Existencia, Precio, Importe FROM avios WHERE Existencia>0 ORDER BY Nombre ASC");
		while($RResAvios=mysql_fetch_array($ResAvios))
		{
			$cadena.='<tr>
						<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF"><input type="checkbox" name="aavio_'.$RResAvios["Id"].'" id="aavio_'.$RResAvios["Id"].'" value="1" '; if($form["aavio_".$RResAvios["Id"]]==1){$cadena.=' checked';}$cadena.='></td>
						<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">'.$A.'</td>
						<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF"><a href="#" onclick="xajax_existencias_avios(\''.$RResAvios["Id"].'\')" class="Ntooltip">'.$RResAvios["Nombre"].'</a></td>
						<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">'.$RResAvios["Color"].'</td>
						<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">'.$RResAvios["Existencia"].'</td>
						<td bgcolor="#CCCCCC" align="right" class="texto" style="border:1px solid #FFFFFF">$ '.number_format($RResAvios["Existencia"]*$RResAvios["Precio"], 2).'</td>
					</tr>';
					$A++;
		}
		$cadena.='	<tr>
...
function agregar_venta($cliente)
{
	...
	$cadena.='		</select></td>
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF"><input type="text" name="nota" id="nota" class="input"></td>
	...
	$cadena.='			</select>
					...
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">';if($_SESSION["perfil"]=='administra'){$cadena.='<input type="checkbox" name="checka" id="checka" value="1">';}$cadena.='</td>
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF"><input type="number" name="metros" id="metros" class="input"></td>
				</tr>
				<tr>
					<td colspan="8" bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF">Comentarios: <br /><textarea name="comentarios" id="comentarios" cols="50" rows="5"></textarea></td>
				</tr>
				<tr>
...
function editar_venta($venta)
{
	...
	$cadena.='		</select></td>
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF"><input type="text" name="nota" id="nota" class="input" value="'.$ResVenta["Nota"].'"></td>
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">
						<select name="apagar" id="apagar">
	...
	$cadena.='			</select>
					...
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">
						<select name="status" id="status">
							<option value=""';if($ResVenta["Status"]==''){$cadena.=' selected';}$cadena.='>SELECCIONE</option>
					...
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">';if($_SESSION["perfil"]=='administra'){$cadena.='<input type="checkbox" name="checka" id="checka" value="1"';if($ResVenta["checka"]==1){$cadena.=' checked';}$cadena.='>';}$cadena.='</td>
...
function editar_pago($pago)
{
	...
	$cadena.='		</select>
					...
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">
						<SELECT name="status" id="status"';if($ResPago["Status"]=='Pagado' AND $_SESSION["perfil"]!='administra'){$cadena.=' disabled';}$cadena.='>
					...
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">';if($_SESSION["perfil"]=='administra'){$cadena.='<input type="checkbox" name="checka" id="checka" value="1"';if($ResPago["checka"]==1){$cadena.=' checked';}$cadena.='>';}$cadena.='</td>
...
$cadena.='				</select>
					...
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF"><input type="number" name="metros" id="metros" class="input" value="'.$ResPago["Metros"].'"></td>
				</tr>
				<tr>
...
function pagos_provedor($provedor=NULL, $accion=NULL, $form=NULL, $anno='todos')
{
	include ("conexion.php");
	...
	{	
		//Agregamos venta
		case 'adcompra':
			mysql_query("INSERT INTO compras (Provedor, Fecha, Apagar, Importe, Comentarios, checka, Metros)
									 VALUES ('".$provedor."',
											 '".$form["anno"]."-".$form["mes"]."-".$form["dia"]."',
											 '".$form["apagar"]."',
											 '".$form["importe"]."',
		...
		//agregar Pago
		case 'adpagoprov':
			mysql_query("INSERT INTO pagos_prov (Provedor, Banco, Cheque, Fecha, Pago, Comentarios, checka, Metros)
									VALUES ('".$provedor."',
											'".$form["banco"]."',
											'".$form["numcheque"]."',
											'".$form["anno"]."-".$form["mes"]."-".$form["dia"]."',
	...
	$cadena.='</select><a href="reportes/reporteprovedor.php?provedor='.$provedor.'" target="_blank"><img src="images/impresora.jpg" border="0"></a> Modelo: <select name="telas" id="telas" onchange="xajax_existencias(this.value)"><option>SELECCIONE</option>';
	$ResTelas=mysql_query("SELECT Id, Nombre FROM telas WHERE Status='0' ORDER BY Nombre ASC");
	while($RResTelas=mysql_fetch_array($ResTelas))
	{
		$cadena.='<option value="'.$RResTelas["Id"].'">'.$RResTelas["Nombre"].'</option>';
	...
	$cadena.='</select></p>
		<p align="center" class="texto">&nbsp;<br />A&ntilde;o: <select name="anno" id="anno" onChange="xajax_pagos_provedor(\''.$provedor.'\', \''.$accion.'\', \''.$form.'\', this.value)">
			<option value="todos">Todos</option>';
	for($i=2000; $i<=(date("Y")+2); $i++)
	{
		$cadena.='<option value="'.$i.'"';if($anno==$i){$cadena.=' selected';}$cadena.='>'.$i.'</option>';
				...
				<tr>
					...
					<form name="fsumacompras" id="fsumacompras">
						<table border="0" style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="center">
							<tr>
								<td bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF">&nbsp;</td>
								<td bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF"><a href="#" onclick="xajax_pagos_provedor(\''.$provedor.'\', \'suma\', xajax.getFormValues(\'fsumacompras\'))">Calcular</a></td>
								<td bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF">';if($suma>0){$cadena.='$ '.number_format($suma,2);}$cadena.='</td>
							...
							<tr>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF"><input type="checkbox" name="allsumacompras" id="allsumacompras" value="1"';if($form["allsumacompras"]==1){$cadena.=' checked';}$cadena.=' onchange="seleccionar_todo_sumacompras()"></td>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">FECHA</td>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">IMPORTE</td>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">SALDO</td>
	...
	$ResCompras=mysql_query("SELECT * FROM compras WHERE Provedor='".$provedor."' AND Historico='0' ORDER BY Fecha ASC");
	...
	$cadena.='		<td valign="top" align="center">
						...
						<table border="0" style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="center">
							<tr>
								<td colspan="4" align="right" class="texto"><a href="#" onclick="xajax_pagos_provedor(\''.$provedor.'\', \'sumapagos\', xajax.getFormValues(\'fsumapagos\'))">Calcular:</a></td>
								<td align="right" class="texto">';if($sumapagos>0){$cadena.='$ '.number_format($sumapagos,2);}$cadena.='</td>
							...
							<tr>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF"><input type="checkbox" name="allsumapagosp" id="allsumapagosp" value="1"';if($form["allsumapagosp"]==1){$cadena.=' checked';}$cadena.=' onchange="seleccionar_todo_sumapagosp()"></td>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">BANCO</td>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">CHEQUE</td>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">FECHA</td>
								<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">PAGO</td>
	...
	$ResPagos=mysql_query("SELECT * FROM pagos_prov WHERE Provedor='".$provedor."' AND Historico='0' ORDER BY Fecha ASC");
	$pago=0;
	while($RResPagos=mysql_fetch_array($ResPagos))
	{
		$pago=$pago+$RResPagos["Pago"];
	...
	$cadena.='			<p></p>
						...
						<table border="0" style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="left">
							...
							<tr>
								<td bgcolor="#000066" colspan="5" align="center" class="texto style="border: 1px solid #FFFFFF">';
	$ResJur=mysql_fetch_array(mysql_query("SELECT Status FROM provedores WHERE Id='".$provedor."' LIMIT 1"));
	if($ResJur["Status"]==0)
	{
		$cadena.='					<a href="#" onclick="xajax_juridico_provedor(\''.$provedor.'\')">ENVIAR A JURIDICO</a>';
	}
	elseif($ResJur["Status"]==1)
	{
		$cadena.='					<a href="#" onclick="xajax_juridico_provedor(\''.$provedor.'\', \'NO\', \'activar\')">ENVIAR A ACTIVO</a>';
	}
	$cadena.='					</td>
							</tr>
							<tr>
								<td bgcolor="#FFFFFF" colspan="5" align="center" class="texto style="border: 1px solid #FFFFFF">';
	$ResJur=mysql_fetch_array(mysql_query("SELECT Status FROM provedores WHERE Id='".$provedor."' LIMIT 1"));
	if($ResJur["Status"]!=2)
	{
		$cadena.='					<a href="#" onclick="xajax_historico_provedor(\''.$provedor.'\')">ENVIAR A HISTORICO</a>';
	}
	elseif($ResJur["Status"]==2)
	{
		$cadena.='					<a href="#" onclick="xajax_historico_provedor(\''.$provedor.'\', \'NO\', \'activar\')">ENVIAR A ACTIVO</a>';
	}
	$cadena.='					</td>
							</tr>
						</table>
...
function agregar_compra($provedor)
{
	...
	$cadena.='		</select></td>
				...
				<tr>
					<td colspan="5" bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF">Comentarios: <br /><textarea name="comentarios" id="comentarios" cols="50" rows="5"></textarea></td>
				</tr>
				<tr>
					<td colspan="5" bgcolor="#FFFFFF" align="center" class="texto" style="border:1px solid #FFFFFF">
...
function editar_compra($compra)
{
	...
	$cadena.='			</select> dias
					...
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">
						<select name="status" id="status">
							<option value=""';if($ResCompra["Status"]==''){$cadena.=' selected';}$cadena.='>Seleccione</option>
					...
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">';if($_SESSION["perfil"]=='administra'){$cadena.='<input type="checkbox" name="checka" id="checka" value="1"';if($ResCompra["checka"]==1){$cadena.=' checked';}$cadena.='>';}$cadena.='</td>
...
function agregar_pago_prov($provedor)
{
	...
	$cadena.='		</select>
					...
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">$ <input type="number" name="pago" id="pago" class="input"></td>
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">';if($_SESSION["perfil"]=='administra'){$cadena.='<input type="checkbox" name="checka" id="checka" value="1">';}$cadena.='</td>
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF"><input type="number" name="metros" id="metros" class="input">
				</tr>
				<tr>
...
$cadena.='				</select>
					...
					<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF"><input type="number" name="metros" id="metros" class="input" value="'.$ResPago["Metros"].'">
				</tr>
				<tr>
					<td colspan="7" bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">
...
function juridico_provedor($provedor, $juridico='NO', $activar=NULL)
{
	include ("conexion.php");
	
	$ResProvedor=mysql_fetch_array(mysql_query("SELECT Id, Nombre FROM provedores WHERE Id='".$provedor."' LIMIT 1"));
	...
	if($juridico=='NO')
	{
		$cadena='<p align="center" class="textomensaje">Esta seguro que desea enviar a Juridico al provedor: '.$ResProvedor["Nombre"].'<br />
					<a href="#" onclick="xajax_juridico_provedor(\''.$provedor.'\', \'SI\')">SI</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick="xajax_pagos_provedor(\''.$provedor.'\')">NO</a></p>';
	}
	elseif($juridico=='SI')
	{
		mysql_query("UPDATE provedores SET Status='1' WHERE Id='".$provedor."'");
		$cadena='<p align="center" class="textomensaje">Se movio el provedor a Juridico</p>';
	}
	}
	elseif($activar=='activar') //mandar a activo
	{
		if($juridico=='NO')
	{
		$cadena='<p align="center" class="textomensaje">Esta seguro que desea enviar a Activo al provedor: '.$ResProvedor["Nombre"].'<br />
					<a href="#" onclick="xajax_juridico_provedor(\''.$provedor.'\', \'SI\', \'activar\')">SI</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" onclick="xajax_pagos_provedor(\''.$provedor.'\')">NO</a></p>';
	}
	elseif($juridico=='SI')
	{
		mysql_query("UPDATE provedores SET Status='0' WHERE Id='".$provedor."'");
		$cadena='<p align="center" class="textomensaje">Se movio el provedor a Activo</p>';
	}
	}
...
function ver_juridico($cliente=NULL, $accion=NULL, $form=NULL)
{
	...
	switch($accion)
	{
		...
		case 'sumaaprovedores':
			$sumaaprovedores=0;
			$ResSumaAProvedores=mysql_query("SELECT Id, Adeudo FROM provedores ORDER BY Id ASC");
			while($RResSumaAProvedores=mysql_fetch_array($ResSumaAProvedores))
			{
				if($form["aprovedor_".$RResSumaAProvedores["Id"]]==1)
				{
	...
	//tabla de provedores juridicos
	$cadena.='<form name="fsumaaprovedores" id="fsumaaprovedores">
				<table border="0" style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="center">
					<table border="0" style="border:1px solid #FFFFFF" cellpadding="3" cellspacing="0" align="center">
					<tr>
						<td colspan="3" align="right" class="texto" style="border: 1px solid #FFFFFF"><a href="#" onclick="xajax_ver_juridico(\'\', \'sumaaprovedores\', xajax.getFormValues(\'fsumaaprovedores\'))">Calcular:</a></td>
						<td align="right" class="texto" style="border: 1px solid #FFFFFF">';if($sumaaprovedores>0){$cadena.='$ '.number_format($sumaaprovedores, 2);}$cadena.='</td>
					</tr>
					<tr>
						<td colspan="4" bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">PROVEDORES EN JURIDICO</td>
					</tr>
					<tr>
						<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF"><input type="checkbox" name="allaprovedores" id="allaprovedores" value="1"';if($form["allaprovedores"]==1){$cadena.=' checked';}$cadena.=' onchange="seleccionar_todo_sumaaprovedores()"></td>
						<td bgcolor="#5263ab" align="center" class="texto3" style="border:1px solid #FFFFFF">#</td>
	...
	$ResProvedoresA=mysql_query("SELECT Id, Nombre, Adeudo FROM provedores WHERE Status='1' ORDER BY Adeudo DESC");
		$A=1; $B=1;
		while($RResProvedoresA=mysql_fetch_array($ResProvedoresA))
		{
			if($B==31)
			...
			$cadena.='<tr>
						<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF"><input type="checkbox" name="aprovedor_'.$RResProvedoresA["Id"].'" id="aprovedor_'.$RResProvedoresA["Id"].'" value="1"'; if($form["aprovedor_".$RResProvedoresA["Id"]]==1){$cadena.=' checked';}elseif($form["allaprovedores"]==0){$cadena.='';}$cadena.='></td>
						<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF">'.$A.'</td>
						<td bgcolor="#CCCCCC" align="center" class="texto" style="border:1px solid #FFFFFF"><a href="#" class="Ntooltip" onclick="xajax_pagos_provedor(\''.$RResProvedoresA["Id"].'\')">'.$RResProvedoresA["Nombre"].'</a></td>
						<td bgcolor="#CCCCCC" align="right" class="texto" style="border:1px solid #FFFFFF">$ '.number_format($RResProvedoresA["Adeudo"],2).'</td>
					</tr>';
			$A++; $B++;
		}
