...
if($_POST["idorden"]){$idorden=$_POST["idorden"];}
elseif($_GET["idorden"]){$idorden=$_GET["idorden"];}

//datos de la orden
$ResOrden=mysql_fetch_array(mysql_query("SELECT * FROM ordenventa WHERE Id='".$idorden."' LIMIT 1"));
...
$ResCliente=mysql_fetch_array(mysql_query("SELECT * FROM clientes WHERE Id='".$ResOrden["Cliente"]."' LIMIT 1"));
...
$pdf->SetY(14);
$pdf->SetX(135);
$pdf->Cell(120,4,'Orde de Venta Num.: '.$ResOrden["NumOrden"],0,0,'L',1);
//fecha
$pdf->SetFillColor(255,255,255);
$pdf->SetFont('Arial','B',10);
$pdf->SetTextColor(000,000,000);
$pdf->SetY(18);
$pdf->SetX(135);
$pdf->Cell(120,4,'Fecha: '.$ResOrden["Fecha"],0,0,'L',1);
//separador
...
//Unidad del Cliente
if($ResOrden["UnidadCliente"]!=0)
{
	$ResUnidadCli=mysql_fetch_array(mysql_query("SELECT Nombre FROM unidades_cliente WHERE Id='".$ResOrden["UnidadCliente"]."' LIMIT 1"));
	$pdf->SetFillColor(255,255,255);
	$pdf->SetFont('Arial','',10);
	$pdf->SetTextColor(000,000,000);
	$pdf->SetY(64);
...
$pdf->SetY(74);
$pdf->SetX(8);
$ResAgente=mysql_fetch_array(mysql_query("SELECT Nombre FROM parametros WHERE Id='".$ResOrden["Agente"]."' LIMIT 1"));
...
$ResPartidas=mysql_query("SELECT * FROM detordenventa WHERE idorden='".$idorden."' ORDER BY Id ASC");
$partidas=1;
$A=1;
...
//partidas en blanco
for($partidas; $partidas<=30; $partidas++)
{
	$pdf->SetFillColor(255,255,255);
		$pdf->SetFont('Arial','',7);
		$pdf->SetTextColor(000,000,000);
...
$y_axis=$y_axis+4;
if($_SESSION["empresa"]==3 AND $_SESSION["sucursal"]==1){$iva=$subtotal*0.11;}
else{$iva=$subtotal*0.16;}
$total=$subtotal+$iva;
